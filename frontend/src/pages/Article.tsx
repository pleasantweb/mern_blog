...
const Article = () => {
  ...
  return (
    ...
    {Array.isArray(blog) && blog.length ? (
     <>
     <h1 className="text-center">{blog[0].title}</h1>
     <div className="px-5 d-flex justify-content-between">
     <p className="text-success" style={{fontSize:'1.2rem'}}>{dateReturn(blog[0].datePosted)}</p>
     <p className="text-capitalize text-primary" style={{fontSize:'1.2rem'}}>{blog[0].category}</p>
     </div>
    
     <img className="rounded mx-auto d-block" style={{minWidth:'90%'}} height='500' width='auto' src={blog[0].blogImage} alt={blog[0].title} />
    <div className="px-5">
        <p>{blog[0].content}</p>
