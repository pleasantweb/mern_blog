...
import { useAllBlogQuery, useBlogLikeMutation, useBlogUnLikeMutation } from "../reduxTool/features/blog/blogApi";
...
const Article = () => {
  ...
  useEffect(()=>{
    ...
    useEffect(()=>{
      if(article && Array.isArray(LikedArticles) && LikedArticles.length){
          let isLiked = LikedArticles.indexOf(article._id)
          if(isLiked > -1){
  ...
  return (
    ...
    <div style={{fontSize:'1.3rem'}} className="d-flex mt-5 px-5">
            <div>{article.likes} 
                {articleLiked ? (
                    <AiFillHeart title="Unlike Article" style={{cursor:'pointer'}} className="mx-2 text-danger" onClick={()=>onLike(false,article._id)} />
                ):(
                 <AiOutlineHeart title="Like Article" style={{cursor:'pointer'}} className="mx-2" onClick={()=>onLike(true,article._id)} />
                )}
